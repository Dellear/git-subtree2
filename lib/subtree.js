...
module.exports = (function () {
    ...
    Subtree._cmd_add = function (args, callback) {
        ...
        if (args.length > 2 && config.repository.startsWith('https://')) {
    ...
    Subtree._cmd_push = function (args, callback) {
        ...
        // git subtree split
        shell.execute('git', ['subtree', 'split', '--rejoin', '--prefix=' + config.localFolder, '--squash'], () => { }, function (data) { process.stdout.write(data) }); 

        // git subtree push
        shell.execute('git', ['subtree', 'push', '--prefix=' + config.localFolder, subtree, config.branch], callback, function(data) { process.stdout.write(data) }); 
    };
